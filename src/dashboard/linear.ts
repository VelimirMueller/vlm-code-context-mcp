...
// ─── Server-side Linear fetch (for trigger endpoint) ─────────────────────
...
export async function fetchAndSyncLinear(
  ...
  if (!token) return { ok: false, error: "No Linear token. Set LINEAR_API_KEY or LINEAR_TOKEN env var." };
  ...
  const gql = async (query: string) => {
    ...
    if (!res.ok) {
      const msg = res.status === 401 ? "Invalid LINEAR_API_KEY" : `Linear API error: ${res.status}`;
      throw new Error(msg);
    }
  ...
  try {
    const issueData = await gql(`{
      issues(first: 250, orderBy: updatedAt, filter: { state: { type: { nin: ["cancelled", "completed"] } } }) {
        nodes {
    ...
    const issues = (issueData.issues?.nodes || []).map((i: any) => ({
      id: i.id,
    ...
    // Also sync to legacy linear_cache for sync status
    const legacyIssues = issues.map((i: any) => ({
      id: i.id, identifier: i.identifier, title: i.title,
      description: i.description, priority: i.priority,
      priorityLabel: i.priorityLabel, status: i.status,
      statusColor: i.statusColor, labels: i.labels,
      projectName: i.projectName, assigneeId: i.assigneeId || "",
      createdAt: i.createdAt, updatedAt: i.updatedAt, url: i.url,
    }));
    syncLinearData(db, { issues: legacyIssues });

    return { ok: true, counts: { issues: issues.length } };
  } catch (err: any) {
